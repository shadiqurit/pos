...
class Login extends MY_Controller {
	...
	{	
		if($this->session->userdata('logged_in')==1){ legacy_redirect(base_url().'dashboard','refresh');	}
		$data = $this->data;
	...
	public function send_otp(){		
		...
		if($this->form_validation->run()==FALSE){
			...
			legacy_redirect(base_url().'login/forgot_password');
		}
		else{
			$email=$this->input->post('email');
			$this->load->model('login_model');//Model
			...
			if($response==true){//Model->Method
				legacy_redirect(base_url().'login/otp');
			}
			else{
				legacy_redirect(base_url().'login/forgot_password');
			}			
	...
	public function change_password(){
		...
		if($this->form_validation->run()==FALSE){
			...
			legacy_redirect(base_url().'login/verify_otp');
		}
		else{
			$otp=$this->input->post('otp');
			$email=$this->input->post('email');
			...
			if($password==$cpassword && $this->session->userdata('email')==$email && $this->session->userdata('otp')==$otp){
				if($this->login_model->change_password($password,$email)){//Model->Method
					...
					legacy_redirect(base_url().'login','refresh');
				}
				else{
					$data=$this->data;
    				$data['email']=$email;
