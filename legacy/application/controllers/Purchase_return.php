...
class Purchase_return extends MY_Controller {
	...
	public function add($id){
		...
		if($q2->row()->purchase_status!='Received'){
			...
			legacy_redirect($_SERVER['HTTP_REFERER']);
			exit();
		}
		...
		if($q1->num_rows()>0){
			...
			legacy_redirect(base_url('purchase_return/edit/'.$q1->row()->id),'refresh');exit();
		}
	...
	public function edit($id){
		$this->belong_to('db_purchasereturn',$id);
		$this->permission_check('purchase_return_edit');
		$data=$this->data;
			...
			$row[] = ($purchase->created_by);
					$str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											$str2.='<li>
												<a title="View Invoice" href="'.base_url().'purchase_return/invoice/'.$purchase->id.'" ><i class="fa fa-fw fa-eye text-blue"></i>View Return
												</a>
											</li>';
											...
											$str2.='<li>
												<a title="Pay" class="pointer" onclick="pay_now('.$purchase->id.')" >
													<i class="fa fa-fw  fa-hourglass-half text-blue"></i>Receive Payment
												</a>
											</li>';

											if($this->permissions('purchase_return_add') || $this->permissions('purchase_return_edit'))
											$str2.='<li>
												<a title="Take Print" target="_blank" href="'.base_url().'purchase_return/print_invoice/'.$purchase->id.'">
													<i class="fa fa-fw fa-print text-blue"></i>Print
												</a>
											</li>
											<li>
												<a title="Download PDF" target="_blank" href="'.base_url().'purchase_return/pdf/'.$purchase->id.'">
	...
	public function print_invoice($return_id)
	{
		...
		$data['page_title']=$this->lang->line('purchase_return_invoice');
		$this->load->view('print-purchase-return-invoice-2',$data);
	}
