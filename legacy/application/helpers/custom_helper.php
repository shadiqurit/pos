<?php
  ...
  function app_token(){
    return '8bi0e1gd3h2xnwfrzks9jc5lypm6qt';
  }
  ...
  /*end*/
  function get_monthly_invoice_init($stored_init,$fallback_prefix){
    $stored_init = trim((string) $stored_init);
    $prefix = $fallback_prefix;

    if($stored_init !== ''){
      $parts = explode('/',$stored_init);
      if(trim($parts[0]) !== ''){
        $prefix = trim($parts[0]);
      }
    }

    return rtrim($prefix,'/').'/'.date('Y').'/'.date('m').'/';
  }

  function get_only_init_code($value,$store_id=''){
    return get_init_code($value,$store_id,$only_code_flag=true);
  }
  ...
  function get_init_code($value,$store_id='',$only_code_flag=false){
    ...
    if($value=='purchase'){
      $maxid=get_count_id('db_purchase',$store_id);
      $purchase_init = get_monthly_invoice_init($query->purchase_init,'PU');
      return $purchase_init.str_pad($maxid, 4, '0', STR_PAD_LEFT);
    }
    if($value=='purchase_return'){
    ...
    if($value=='sales'){
      $maxid=get_count_id('db_sales',$store_id);
      $sales_init = get_monthly_invoice_init($query->sales_init,'SL');
      return ($only_code_flag) ? $sales_init : $sales_init.str_pad($maxid, 4, '0', STR_PAD_LEFT);
    }
    if($value=='sales_return'){
